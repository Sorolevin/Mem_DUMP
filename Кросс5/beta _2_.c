	...
	hg  = 	  *y < LN	 ?  *y : hg    ;
		...
		for(  mem = *y, lvl = 0    , rd = 0; lvl <= lng     ;      lvl++  ,mem++	  ){
			 	  ...
			 	  for(brd = 0  ;   ( brd  <= hg )  ; brd++      ){
					...
					for( shift = *x,   pnt = 0, stp = 0;   stp  <= LN  ;stp++ , shift++    ){
				   	  
				   	 
				   	  
				   	  
					if ( 	  checking_space_null ( board  ,   shift,  mem  ,  brd   )   == 1    || 
					
					
					
					(  isalpha (  (unsigned char)  board[ *x  + 1][mem - 1])  ||    isalpha( (unsigned char)    board[  *x + 1][mem + 1])        )      ){
...
void  removing_mask_horizont (int *x , int *y, int lng  , char board[][Y] , char *content[PL] ){
	...
	for(  mem = *x, lvl = 0    , rd = 0; lvl <= lng     ;      lvl++  ,mem++	  ){
			...
			for(brd = 0  ;    brd  <= hg  ; brd++           ){
				...
				for( shift = *y   ,  pnt = 0, stp = 0;   stp  <= LN  ;stp++ , shift++   ){
				   	  
				   	  
				if ( 	  checking_space_one( board  , mem    ,   shift ,  brd  )   == 1  || 
				
				
				
				
				(  isalpha( (unsigned char)   board[  mem + 1 ][ *y  + 1  ]   )  ||  isalpha( (unsigned char)   board[  mem + 1 ][ *y  - 1  ])        )         ){
						 pnt = 1;
